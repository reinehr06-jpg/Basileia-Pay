# 🚀 Código Fonte Completo - REVISADO v2
Este arquivo contém 100% do código fonte revisado do projeto, incluindo as últimas atualizações de gateways dinâmicos e webhook idempotente.
...
class AuthenticateApi
{
    ...
    /**
     * Load the company's default gateway configuration into request attributes.
     * Uses request attributes instead of global config() to avoid race conditions
     * in PHP-FPM where config is shared across concurrent requests.
     */
    private function loadGatewayConfig($company): void
    {
        ...
        // Map common keys from DB
        $configs = $gateway->configs->pluck('value', 'key')->toArray();

        $gatewayConfig = match ($type) {
            'asaas' => [
                'driver' => 'asaas',
                'api_key' => $configs['api_key'] ?? config('services.asaas.api_key'),
                'environment' => $configs['environment'] ?? config('services.asaas.environment'),
                'webhook_token' => $configs['webhook_token'] ?? config('services.asaas.webhook_token'),
            ],
            'stripe' => [
                'driver' => 'stripe',
                'key' => $configs['public_key'] ?? config('services.stripe.key'),
                'secret' => $configs['secret_key'] ?? config('services.stripe.secret'),
            ],
            'mercadopago' => [
                'driver' => 'mercadopago',
                'access_token' => $configs['access_token'] ?? config('services.mercado_pago.access_token'),
            ],
            default => null,
        };

        // Store in request attributes — safe for concurrent requests
        $this->request->attributes->set('gateway_config', $gatewayConfig);
        $this->request->attributes->set('gateway_type', $type);
    }
}
...
class CardController extends AbstractCheckoutController
{
    ...
    public function process(string $uuid, Request $request)
    {
        $resource = \App\Services\CheckoutService::findResource($uuid);
        ...
        try {
            // Validation and processing using CardPaymentService
            // Note: In a real scenario, we'd use FormRequests
            $request->validate([
            ...
            $result = $this->cardService->charge($request->all(), [
                'name' => $request->input('customerData.name'),
                'email' => $request->input('customerData.email'),
                'document' => $request->input('customerData.document'),
            ]);
            ...
            return response()->json([
                ...
                'status' => 'success',
                'gatewayId' => $result['gatewayId'],
                'redirectUrl' => route('checkout.card.success', $uuid),
            ]);

        } catch (\Exception $e) {
            Log::error('CardController: Erro ao processar cartão', [
    ...
    protected function getFallbackView($transaction, $asaasPayment, $customerData, $pixData, $plano, $ciclo, $i18n, $request)
    {
...
class BoletoController extends AbstractCheckoutController
{
    ...
    public function process(string $uuid, Request $request)
    {
        ...
        try {
            $result = $this->boletoService->charge($request->all(), [
                'name' => $request->input('customerData.name'),
                'email' => $request->input('customerData.email'),
                'document' => $request->input('customerData.document'),
            ]);
            ...
            Log::error('BoletoController: Erro ao gerar boleto', [
                'transaction_uuid' => $uuid,
                'error' => $e->getMessage(),
            ]);
...
class PixController extends AbstractCheckoutController
{
    ...
    public function process(string $uuid, Request $request)
    {
        ...
        try {
            ...
            $result = $this->pixService->charge($input, $customerData);
            ...
            Log::info('PixController: QR Code gerado', [
                'transaction_id' => $transaction->id,
            ...
            Log::error('PixController: Erro ao gerar PIX', [
                'transaction_uuid' => $uuid,
                'error' => $e->getMessage(),
            ]);
 ...
 *
 * Base class for payment method specific controllers (Card, Boleto, PIX).
 * Consolidates 70-90% duplicate code from 6 different checkout controllers.
 *
 * Extracted common responsibilities:
 * - Transaction/Subscription lookup by UUID
 * - Asaas payment fetching with local fallback
 * - Customer data building
 * - Transaction creation (when not exists)
 * - i18n loading
 * - Status polling with auto-update
...
abstract class AbstractCheckoutController extends Controller
{
    ...
    public function __construct(
        ...
        WebhookNotifierService $webhookNotifier
    ) {
    ...
    public function show(string $uuid, Request $request)
    {
        ...
        Log::info(static::class . ': Renderizando checkout', [
            'asaas_payment_id' => $asaasPaymentId,
            'transaction_uuid' => $resource->uuid,
        ]);
        ...
        $pixData = $this->needsPixData() ? CheckoutService::getPixDataIfNeeded(
            $this->asaasService,
            $asaasPaymentId
        ) : [];
        ...
        $htmlPath = public_path('checkout-app/checkout.html');
        if (!file_exists($htmlPath)) {
            return $this->getFallbackView($transaction, $asaasPayment, $customerData, $pixData, $plano, $ciclo, $i18n, $request);
        }

        $html = file_get_contents($htmlPath);
        $checkoutData = CheckoutService::buildCheckoutData($transaction, $asaasPayment, $transaction->uuid, $request);
        $injection = "<script>window.CHECKOUT_DATA = " . json_encode($checkoutData) . ";</script>";
        $html = str_replace('<head>', "<head>\n    " . $injection, $html);
    ...
    /**
     * Process payment (implemented by child)
     */
    abstract public function process(string $uuid, Request $request);
    ...
    public function status(string $uuid)
    {
        ...
        CheckoutService::checkAndUpdateStatus(
            $transaction,
            $this->asaasService,
            [$this->getPaymentService(), 'mapStatus'],
            [$this->getPaymentService(), 'isPaid']
        );
    ...
    public function success(string $uuid)
    {
        ...
        return view($this->getSuccessViewName(), [
            'transaction' => $resource,
        ]);
    }
     ...
     * @return string 'credit_card', 'pix', or 'boleto'
     */
    abstract protected function getPaymentMethod(): string;

    /**
     * Get the payment service instance
     * @return object Service with charge(), mapStatus(), isPaid() methods
     */
    abstract protected function getPaymentService();

    /**
     * Get the view name for checkout page
     * @return string Blade view name
     */
    abstract protected function getViewName(): string;

    /**
     * Get the view name for success page
     * @return string Blade view name
     */
    abstract protected function getSuccessViewName(): string;

    /**
     * Get the transaction source identifier
     * @return string Source value for transaction record
     */
    abstract protected function getSource(): string;

    /**
     * Get default billing type for fallback
     * @return string 'CREDIT_CARD', 'PIX', or 'BOLETO'
     */
    abstract protected function getDefaultBillingType(): string;

    /**
     * Whether this payment method needs PIX data
     */
    abstract protected function needsPixData(): bool;

    /**
     * Get fallback view when SPA HTML doesn't exist
     */
    abstract protected function getFallbackView($transaction, $asaasPayment, $customerData, $pixData, $plano, $ciclo, $i18n, $request);
}
...
class EventCheckoutController extends Controller
{
    ...
    public function process(Request $request, string $slug)
    {
        ...
        try {
            ...
            Log::error('EventCheckout: Error', ['error' => $e->getMessage()]);
...
class WebhookController extends Controller
{
    private function getAsaasIpWhitelist(): array
    {
        ...
        // Fallback padrão — documentar que deve ser configurado via .env
...
use App\Models\Subscription;
use App\Services\AsaasPaymentService;
use App\Services\Payment\PaymentStatusMapper;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Str;
use Illuminate\Http\Request;

/**
 * CheckoutService - Centralizes all common checkout logic
 *
 * Extracted from 6 duplicate checkout controllers:
 * - CheckoutController
 * - BasileiaCheckoutController
 * - AsaasCheckoutController
 * - CardCheckoutController
 * - BoletoCheckoutController
 * - PixCheckoutController
 *
 * Responsibilities:
 * - Find transaction/subscription by UUID
 * - Fetch Asaas payment with fallback to local data
 * - Build customer data array
 * - Create transaction from payment data
 * - Load i18n translations
 * - Check and update payment status (polling)
 */
class CheckoutService
{
    /**
     * Find transaction or subscription by UUID
     */
    ...
    /**
     * Get Asaas payment data with local fallback
     *
     * @param AsaasPaymentService $asaasService
     * @param Transaction|Subscription $resource
     * @param string $asaasPaymentId
     * @param string $defaultBillingType Default billing type for fallback
     * @return array Asaas payment data
     */
    public static function getAsaasPaymentWithFallback(
        AsaasPaymentService $asaasService,
        $resource,
        string $asaasPaymentId,
        string $defaultBillingType = 'CREDIT_CARD'
    ): array {
        try {
            $asaasPayment = $asaasService->getPayment($asaasPaymentId);
            if ($asaasPayment) {
                return $asaasPayment;
            }
        } catch (\Exception $e) {
            Log::warning('CheckoutService: Failed to fetch from Asaas, using fallback', [
                'asaas_payment_id' => $asaasPaymentId,
                'error' => $e->getMessage(),
            ]);
        }
        ...
        Log::info('CheckoutService: Using local fallback for payment data', [
            'asaas_payment_id' => $asaasPaymentId,
        ...
        $paymentMethod = $methodMap[$resource->payment_method ?? 'credit_card'] ?? $defaultBillingType;
    ...
    /**
     * Build customer data array from Asaas payment and/or transaction
     *
     * @param array $asaasPayment
     * @param Transaction|Subscription $transaction
     * @return array Customer data with keys: name, email, phone, document, address[]
     */
    public static function buildCustomerData(array $asaasPayment, $transaction): array
    {
        $customer = $asaasPayment['customer'] ?? [];
    ...
    /**
     * Create a transaction from payment data if it doesn't exist
     *
     * @param array $asaasPayment
     * @param Transaction|Subscription $transaction (may be empty model)
     * @param string $asaasPaymentId
     * @param string $source Source identifier (e.g., 'basileia_vendas', 'checkout')
    ...
    public static function createTransactionIfNotExists(
        ...
        // If transaction already exists (has ID), just return it
        if ($transaction->exists && $transaction->id) {
            return $transaction;
        }

        $customerData = self::buildCustomerData($asaasPayment, $transaction);
        $plano = $request->get('plano', $asaasPayment['description'] ?? 'Plano');
        $ciclo = $request->get('ciclo', 'mensal');
        $companyId = \App\Models\Company::first()?->id ?? 1;
        $billingType = $asaasPayment['billingType'] ?? 'CREDIT_CARD';
        $paymentMethod = PaymentStatusMapper::mapPaymentMethod($billingType);
    ...
    /**
     * Load i18n translations for checkout
     *
     * @param Request $request
     ...
     * @return array
     */
    public static function getPixDataIfNeeded(AsaasPaymentService $asaasService, string $asaasPaymentId): array
    {
        $pixData = [];
        if ($asaasPaymentId) {
            $pixData = $asaasService->getPixQrCode($asaasPaymentId) ?? [];
        }
        return $pixData;
    }

    /**
     * Check pending transaction status and update if paid
     *
     * @param Transaction $transaction
     * @param AsaasPaymentService $asaasService
     * @param callable|null $mapStatus Function to map gateway status to internal status
     * @param callable|null $isPaid Function to check if status means paid
     * @return void
     */
    public static function checkAndUpdateStatus(
        Transaction $transaction,
        AsaasPaymentService $asaasService,
        ?callable $mapStatus = null,
        ?callable $isPaid = null
    ): void {
        if ($transaction->status !== 'pending' || !$transaction->asaas_payment_id) {
            return;
        }

        $asaasPayment = $asaasService->getPayment($transaction->asaas_payment_id);
        if (!$asaasPayment) {
            return;
        }

        $gatewayStatus = $asaasPayment['status'] ?? 'PENDING';

        if ($mapStatus) {
            $status = $mapStatus($gatewayStatus);
        } else {
            $status = PaymentStatusMapper::mapStatus($gatewayStatus);
        }
        ...
        if ($isPaid) {
            $paidAt = $isPaid($gatewayStatus) ? now() : null;
        } else {
            $paidAt = PaymentStatusMapper::isPaid($gatewayStatus) ? now() : null;
        }
        ...
        Log::info('CheckoutService: Transaction status updated', [
            'transaction_id' => $transaction->id,
            'old_status' => 'pending',
            'new_status' => $status,
        ]);
        ...
        try {
            app(\App\Services\WebhookNotifierService::class)->notify($transaction);
        } catch (\Exception $e) {
            Log::error('CheckoutService: Failed to send webhook notification', [
                'transaction_id' => $transaction->id,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    /**
     * Build checkout data array for SPA/HTML injection
     *
     * @param Transaction|Subscription $resource
     * @param array $asaasPayment
     * @param string $uuid
     ...
     * @return array
     */
    public static function buildCheckoutData($resource, array $asaasPayment, string $uuid, Request $request): array
    {
        return [
            'uuid' => $uuid,
            'amount' => $asaasPayment['value'] ?? ($resource->amount ?? 0),
            'description' => $resource instanceof Subscription
                ? $resource->plan_name
                : ($resource->description ?: 'Plano Premium'),
            'customerName' => $resource->customer_name ?? ($resource->customer?->name ?? ''),
            'customerEmail' => $resource->customer_email ?? ($resource->customer?->email ?? ''),
            'csrfToken' => csrf_token(),
            'step' => 1,
    ...
    /**
     * Create a transaction from secure tokenization redirect parameters.
     * Used by EnforceSecureTokenization middleware when a transaction doesn't exist yet.
     *
     * @param array $data [asaas_payment_id, url_params]
    ...
    public static function createTransactionFromRedirect(array $data): ?Transaction
    {
        try {
            ...
            return Transaction::create([
                ...
                'payment_method' => 'credit_card',
                'status' => 'pending',
                'customer_name' => preg_replace('/[^a-zA-ZÀ-ÿ\s]/', '', $params['cliente'] ?? ''),
                'customer_email' => filter_var($params['email'] ?? '', FILTER_VALIDATE_EMAIL)
                    ? $params['email'] : '',
                ...
                'metadata' => [
                    ...
                    'venda_id' => '',
                    'hash' => '',
                    'redirect_source' => 'secure_tokenization',
                ],
            ]);
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('CheckoutService: Failed to create transaction from redirect', [
                'error' => $e->getMessage(),
...
class AsaasPaymentService
{
    ...
    private function request(string $method, string $endpoint, array $data = []): array
    {
        ...
        $apiKey = GatewayResolver::resolveApiKey();
    ...
    /**
     * Process a credit card payment using a stored card token (for subscriptions).
     */
...
================================================================================
FILE: app/Services/PaymentStatusMapper.php
================================================================================
<?php
...
/**
 * Centraliza o mapeamento de status de pagamento entre gateways e o sistema interno.
 * Utilizado por CardPaymentService, BoletoPaymentService, PixPaymentService e controllers.
 *
 * Refatorado de:
 *   - CardPaymentService::mapStatus() / CardPaymentService::isPaid()
 *   - BoletoPaymentService::mapStatus() / BoletoPaymentService::isPaid()
 *   - PixPaymentService::mapStatus() / PixPaymentService::isPaid()
 *   - DefaultVendorController::mapStatus() / DefaultVendorController::mapPaymentMethod()
 *   - AsaasCheckoutController::mapPaymentMethod()
 */
class PaymentStatusMapper
{
    /**
     * Mapeia o status do gateway para o status interno do sistema.
     */
    ...
    /**
     * Verifica se o status do gateway indica pagamento confirmado/aprovado.
     */
    ...
    /**
     * Mapeia o tipo de billing do gateway para o método de pagamento interno.
     */
...
/**
 * Centraliza a resolução do gateway de pagamento e configuração da API Key.
 *
 * Refatorado de código duplicado em:
 *   - DefaultVendorController::handle()
 *   - BasileiaCheckoutController (lógica similar)
 *   - AsaasCheckoutController (lógica similar)
 */
class GatewayResolver
{
    /**
     * Resolve e configura a API Key do gateway ativo.
     * Se a configuração services.asaas.api_key estiver vazia, busca o gateway
     * padrão ativo no banco de dados e configura automaticamente.
     *
     * @return string A API Key configurada
     * @throws \RuntimeException Se nenhuma API Key puder ser configurada
     */
    public static function resolveApiKey(): string
    {
        ...
        if (empty($apiKey)) {
            $defaultGateway = Gateway::where('status', 'active')
                ->where('is_default', true)
                ->first()
            ...
            if ($defaultGateway) {
                ...
                Log::info('GatewayResolver: API Key resolved from default gateway', [
                    'gateway_id' => $defaultGateway->id,
                    'gateway_type' => $defaultGateway->type,
                ]);
            }
        }

        if (empty($apiKey)) {
            Log::warning('GatewayResolver: No API Key could be resolved');
    ...
    /**
     * Obtém o gateway padrão ativo.
     */
    public static function getDefaultGateway(): ?Gateway
    {
        return Gateway::where('status', 'active')
            ->where('is_default', true)
            ->first()
            ?? Gateway::where('status', 'active')->first();
    }
    ...
    /**
     * Resolve e retorna uma instância do gateway configurado.
     *
     * @param string|null $type Tipo do gateway (null = padrão ativo)
     * @return PaymentGatewayInterface
     */
    public static function resolveGateway(?string $type = null): PaymentGatewayInterface
    {
        $gatewayType = $type ?? strtolower(config('services.payment_gateway', 'asaas'));

        return match ($gatewayType) {
            'asaas' => new AsaasGateway(self::resolveApiKey()),
            'stripe' => new \App\Services\Gateway\StripeGateway(self::resolveApiKey()),
            default => throw new \RuntimeException("Gateway '{$gatewayType}' não suportado."),
        };
    }
}

================================================================================
FILE: app/Services/Gateway/AdyenGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/CustomGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/WorldpayGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/BlueSnapGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/PayPalGateway.php
================================================================================
<?php
...
class GatewayFactory
{
     ...
     * Uses GatewayResolver to get the API key from the database.
     *
     * @return \App\Services\Gateway\PaymentGatewayInterface
     */
    public static function create(): PaymentGatewayInterface
    {
        $gatewayType = config('services.payment_gateway', 'asaas');

        return match ($gatewayType) {
            'asaas' => app(AsaasGateway::class),
            'stripe' => app(StripeGateway::class),
            default => throw new \Exception("Gateway {$gatewayType} não implementado."),
        };
    }

    /**
     * Create a gateway instance by type, using the API key from the database.
     *
     * @param string $type 'asaas', 'stripe', etc.
     * @return GatewayInterface
     */
    public static function createFromTransaction(string $type): GatewayInterface
    {
        $type = strtolower($type);
        $apiKey = GatewayResolver::resolveApiKey();

        return match ($type) {
            'asaas' => new AsaasGateway($apiKey),
            'stripe' => new StripeGateway($apiKey),
            default => throw new \Exception("Gateway type '{$type}' not supported."),
        };
    }

    /**
     * Create a gateway instance by type.
     * This method is used when you need a specific gateway.
     *
     * @param string $type 'asaas', 'stripe', etc.
     * @return GatewayInterface
     */
...
================================================================================
FILE: app/Services/Gateway/AirwallexGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/RazorpayGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/MollieGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/SkrillGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/VerifoneGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/KlarnaGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/FlutterwaveGateway.php
================================================================================
<?php
 ...
 * Implements PaymentGatewayInterface for high-level payment operations.
 */
class AsaasGateway implements PaymentGatewayInterface
{
    ...
    {
        $key = $this->apiKey ?? GatewayResolver::resolveApiKey();
    ...
    /**
     * Process credit card payment (one-time)
     */
    ...
    public function chargeViaBoleto(array $input, string $customerId): array
    {
        ...
        if (!$response->successful()) {
            throw new \Exception("Asaas Boleto charge error: " . json_encode($json));
        }
        ...
        return [
            ...
            'status' => $json['status'],
            'amountCharged' => $amountBRL,
            'boletoUrl' => $json['bankSlipUrl'] ?? '',
            'boletoBarcode' => $json['identificationField'] ?? '',
            'raw' => $json,
        ];
    }

    // -------------------------------------------------------------------------
    // Additional methods for GatewayInterface compatibility (legacy support)
    // These wrap the high-level methods above
    // -------------------------------------------------------------------------
    ...
    public function createPayment(array $data): array
    {
        $customerId = $this->createCustomer([
            ...
            'address' => $data['customer']['address'] ?? '',
            'address_number' => $data['customer']['addressNumber'] ?? '',
        ]);

        $input = [
            'amountBRL' => $data['value'],
            'description' => $data['description'],
            'remoteIp' => $data['ip'] ?? request()->ip(),
            'cardToken' => $data['credit_card']['number'] ?? '',
            'cardHolderName' => $data['credit_card']['holderName'] ?? '',
            'cardExpiry' => ($data['credit_card']['expiryMonth'] ?? '') . '/' . ($data['credit_card']['expiryYear'] ?? ''),
            'cardCvv' => $data['credit_card']['ccv'] ?? '',
        ];

        $result = $this->charge($input, $customerId);
        return $result['raw'] ?? [];
    }
    ...
    {
        $url = "{$this->getBaseUrl()}{$endpoint}";
        $apiKey = $this->apiKey ?? GatewayResolver::resolveApiKey();

        try {
            if (empty($apiKey)) {
                Log::warning('AsaasGateway: ASAAS_API_KEY not configured');
                throw new \RuntimeException('Gateway not configured (ASAAS_API_KEY is empty)');
            }
            ...
            Log::error('AsaasGateway: Exception', [
                'url' => $url,
                'error' => $e->getMessage(),
            ]);
            throw $e;
...
================================================================================
FILE: app/Services/Gateway/CheckoutComGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/RapydGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/MercadoPagoGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/BraintreeGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/StripeGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/SquareGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/AuthorizeNetGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/PagSeguroGateway.php
================================================================================
<?php
...
================================================================================
FILE: app/Services/Gateway/PayoneerGateway.php
================================================================================
<?php
