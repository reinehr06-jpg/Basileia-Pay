...
export function Sidebar() {
  ...
  return (
    <aside className="w-[260px] h-screen p-2.5 flex flex-col z-20 shrink-0">
      <div className="flex-1 bg-white/70 backdrop-blur-xl border border-border rounded-[18px] flex flex-col overflow-hidden shadow-xl shadow-brand/5">
        ...
        <div className="p-4 pb-0 flex items-center gap-2">
          <div className="w-7 h-7 bg-gradient-to-br from-brand to-brand-accent rounded-lg flex items-center justify-center shadow-md shadow-brand/20">
            <span className="text-white font-black text-base">B</span>
          </div>
          <span className="text-ink font-black text-[15px] tracking-tight">Basileia Pay</span>
        </div>

        {/* Nav - Maximum Density */}
        <nav className="flex-1 overflow-y-auto px-1.5 py-3 no-scrollbar space-y-4">
          {menuGroups.map((group) => (
            <div key={group.title} className="space-y-0.5">
              <p className="px-2.5 text-[8.5px] font-black text-slate uppercase tracking-widest mb-1 opacity-30">
                {group.title}
              </p>
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      href={item.href}
                      className={cn(
                        "group flex items-center gap-2 px-2.5 py-1.5 rounded-lg transition-all duration-300 relative overflow-hidden",
                        isActive 
                          ? "bg-gradient-to-r from-brand to-brand-accent text-white shadow-md shadow-brand/20 h-[42px]" 
                          : item.isSpecial && pathname.startsWith(item.href)
                            ? "bg-brand text-white shadow-md shadow-brand/20 h-[42px]"
                            : "text-slate hover:bg-brand-soft hover:text-brand h-[40px]"
                      )}
                    >
                      ...
                      <span className={cn(
                        "font-bold text-[12px] relative z-10",
