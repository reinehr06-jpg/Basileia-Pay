...
<head>
    ...
    <title><?php echo e($plano); ?> - Checkout Basiléia</title>
...
<body class="bg-dark text-white font-sans overflow-x-hidden" x-data="checkoutFlow()">
    <script>
        function checkoutFlow() {
            const uuid = <?php echo json_encode($transaction->uuid); ?>;
            return {
                ...
                cardHolder: '',
                cardBrand: 'default',
                vendorName: <?php echo json_encode($transaction->vendor->name ?? ''); ?>,
                vendorEmail: <?php echo json_encode($transaction->customer_email ?? ''); ?>,
                vendorDoc: <?php echo json_encode($transaction->customer_document ?? ''); ?>,
                mobileSummaryOpen: false,
                isExpired: false,
                originalAmount: <?php echo json_encode(number_format($transaction->amount ?? 0, 2, '.', '')); ?>,
    ...
    <div class="checkout-wrapper">
        ...
        <div class="order-summary" :class="{ 'mobile-open': mobileSummaryOpen }">
            ...
            <div class="summary-card">
                ...
                <h1 class="plan-title" style="color: white;"><?php echo e($plano); ?></h1>
            ...
            <div style="text-align: center; color: #a99fbb; font-size: 11px; opacity: 0.5;">
                Basiléia Church &copy; <?php echo e(date('Y')); ?> - Todos os direitos reservados
            </div>
        </div>
        ...
        <div class="book-container">
            ...
            <div class="layer" x-show="step === 1 || step === 2">
                ...
                <?php if(($asaasPayment['billingType'] ?? 'PIX') === 'PIX'): ?>
                    <!-- PIX FLOW -->
                    <div class="pix-container">
                        ...
                        <div class="pix-qr-box" style="border: 2px solid var(--primary); box-shadow: 0 15px 35px rgba(124, 58, 237, 0.2); background: white; padding: 15px; border-radius: 20px; display: inline-block; margin-bottom: 20px;">
                            <img src="data:image/png;base64,<?php echo e($pixData['encodedImage'] ?? ''); ?>" alt="QR Code PIX" style="width: 180px; height: 180px; display: block;">
                        </div>
                        ...
                        <div style="text-align: left; margin-bottom: 15px;">
                            ...
                            <div class="pix-payload" id="pixPayload"><?php echo e($pixData['payload'] ?? ''); ?></div>
                        </div>
                ...
                <?php else: ?>
                    ...
                    <form id="paymentForm" method="POST" action="<?php echo e(route('checkout.process', $transaction->uuid)); ?>" x-show="step === 2" x-transition:enter="layer-transition" x-transition:enter-start="layer-enter-start" x-transition:enter-end="layer-enter-end" x-transition:leave="layer-transition" x-transition:leave-start="layer-leave-start" x-transition:leave-end="layer-leave-end" x-cloak>
                        ...
                        <button type="button" class="btn-secondary" @click="step = 1; localStorage.setItem('checkout_step_' + '<?php echo e($transaction->uuid); ?>', 1)" style="margin-top: 10px; width: 100%;" x-show="!processing">
            ...
            <div class="layer" x-show="step === 3" 
                ...
                <div style="background: #f8fafc; border-radius: 16px; padding: 20px; margin-top: 20px;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 10px; font-size: 13px; color: #64748b;">
                        <span>Transação</span>
                        <span style="font-weight: 700; color: #1e293b;">#<?php echo e(strtoupper(substr($transaction->uuid, 0, 8))); ?></span>
                    </div>
                    <div style="display: flex; justify-content: space-between; font-size: 13px; color: #64748b;">
                        <span>Data</span>
                        <span style="font-weight: 700; color: #1e293b;"><?php echo e(date('d/m/Y H:i')); ?></span>
                    </div>
                </div>
...
<?php /**PATH /Users/viniciusreinehr/.gemini/antigravity/scratch/CheckOutFINAL/resources/views/checkouts/basileia-vendor/index.blade.php ENDPATH**/ ?>